...
const Login = () => {
  ...
  const FormContainer = ({ children, isSignUp }) => (
    <div className={`absolute top-0 h-full transition-all duration-500 ease-in-out
      ${isMobile 
        ? `w-full ${isRightPanelActive ? (isSignUp ? 'translate-x-0' : 'translate-x-full') : (isSignUp ? 'translate-x-full' : 'translate-x-0')}`
        : `w-1/2 ${isRightPanelActive ? (isSignUp ? 'translate-x-full opacity-100 z-50' : 'translate-x-full') : (isSignUp ? 'opacity-0 z-10' : '')}`
      }`}>
      {children}
    </div>
  );
              ...
              <div className="w-full mt-6">
                ...
                <GoogleLogin
                  onSuccess={response => {
                    ...
                    dispatch(googleLogin({ uid, name, email })).then((response) => {
                      if (response.payload) navigate('/');
                    });
                  }}
