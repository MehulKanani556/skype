...
import { FaSearch, FaRegUser, FaCommentDots, FaPhone, FaUsers, FaBell, FaDownload, FaPaperclip, FaMicrophone, FaRegSmile, FaPlusCircle, FaFilePdf, FaFileExcel, FaFileWord, FaFilePowerpoint, FaFileArchive, FaArrowDown, } from "react-icons/fa";
import { HiOutlineReply } from "react-icons/hi";
import { PiDotsThreeVerticalBold, PiDotsThreeBold } from "react-icons/pi";
import { VscCopy } from "react-icons/vsc";
import { MdCallEnd, MdOutlineModeEdit, MdPhoneEnabled, MdGroupAdd } from "react-icons/md";
...
import { IoCameraOutline, IoCheckmarkCircleOutline, IoCheckmarkDoneCircle, IoCheckmarkDoneCircleOutline } from "react-icons/io5";
  ...
  useEffect(() => {
    if (searchInput) {
      ...
      if (selectedTab === "Unread") {
        setFilteredUsers(allMessageUsers.filter(user =>
          user.messages.some(message => message.status === "sent" || message.status === "delivered")
        ));
      } else {
                    ...
                    <div className="w-10 h-10 rounded-full bg-gray-300 overflow-hidden flex items-center justify-center">
                      {item?.photo && item.photo !== "null" ? (
                        <img src={`${IMG_URL}${item.photo.replace(
                          /\\/g,
                          "/"
                        )}`} alt="Profile" className="object-cover rounded-full" />
                      ) : (
                ...
                {isVideoCalling && (
                  <div className="flex gap-2 ">
                    <button
                      ...
                      className={`px-4 py-2 rounded h-10 ${isCameraOn ? "bg-green-500" : "bg-red-500"
                        } text-white`}
                    >
                    ...
                    <button
                      ...
                      className={`px-4 py-2 rounded h-10 ${isMicrophoneOn ? "bg-green-500" : "bg-red-500"
                        } text-white`}
                    >
